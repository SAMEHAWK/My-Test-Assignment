...
本项目实现了主动式布娃娃的完整链路：WASD 移动 → 轻击晃动 → 重击局部物理 → 击倒全身布娃娃 → 起身恢复。核心架构采用层次状态机 + 双骨架物理，目前已可运行。

**已知待改进：**
- **重击表现**：局部 ragdoll 回融动画的流畅度、受击链的力传播衰减、Stagger 与物理的融合过渡仍有调整空间，暂未找到满意的参数组合。
- **动画机**：当前 Animator Controller 的层结构是随功能迭代堆出来的，层权重管理分散在代码里。计划将其重构为独立的动画机模块，把层切换、CrossFade 调度、overlay 生命周期收敛到一处。
- **碰撞反馈**：`CharacterHurtbox` 已预留了部位伤害倍率，但尚未按肢体差异化利用，受击观感还有层次提升空间。

以上方向目前大多只有模糊的想法，还没找到具体解法。

全部代码与文档在 AI（Cursor + Deepseek V4 pro）辅助下完成，遵循「计划 → 确认 → 实现」协作流程。
